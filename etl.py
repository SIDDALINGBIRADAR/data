#basic imports - karplexus.azurecr.io/airflow:1.0.0
import base64
import json
import logging
import signal
...
class TinkETL:
    def __init__(
        ...
        logger.info(
            ...
            "read_chunk_size=%s | write_chunk_size=%s | dlq_version=%s | schema_agnostic=%s | mode=single_connection",
    ...
    def _enhance_conn_str(self, conn_str: str) -> str:
        ...
        db_ssl_mode = get_var("db_ssl_mode", "disable")
    ...
    def _get_source_conn(self):
        """Create a fresh connection to the Source DB. Maintains connection for duration of context."""
        conn = None
        try:
            # Create a brand new connection directly
            conn = psycopg2.connect(self.source_conn_str)
            yield conn
        finally:
            if conn:
                try:
                    conn.close()  # Close it immediately after use
                except Exception:
                    logger.error("Error closing source connection", exc_info=True)

    @contextmanager
    def _get_target_conn(self):
        """Create a fresh connection to the Target DB. Closes immediately after context exit."""
        conn = None
        try:
            # Create a brand new connection directly
            conn = psycopg2.connect(self.target_conn_str)
            yield conn
        finally:
            if conn:
                try:
                    ...
                    logger.error("Error closing target connection", exc_info=True)
    ...
    def extract_data_generator(self, table_name: str, last_updated_filter: Optional[datetime] = None) -> Iterator[pd.DataFrame]:
        ...
        if watermark_col and last_updated_filter:
            query = sql.SQL(
                "{base} WHERE {watermark_col} >= %s OR {watermark_col} IS NULL ORDER BY {watermark_col},{id_column}"
            ).format(base=query, watermark_col=sql.Identifier(watermark_col), id_column=sql.Identifier(id_column))
            params = (last_updated_filter,)
        else:
            if watermark_col:
                query = sql.SQL("{base} ORDER BY {watermark_col},{id_column}").format(
                    base=query, watermark_col=sql.Identifier(watermark_col), id_column=sql.Identifier(id_column)
                )
            params = None
    ...
    def _decrypt_column_batch(self, ciphertext_array: np.ndarray) -> np.ndarray:
        ...
        for i, ciphertext_b64 in enumerate(ciphertext_array):
            try:
                ...
                # Just decode to string. Let Postgres handle the JSON.
    ...
    def _load_with_fixed_chunks(self, df: pd.DataFrame, table_name: str, table_config: Dict[str, Any]):
        ...
        # Apply Vectorized Transform ONCE before chunking ---
...
def tink_multi_table_etl_json_dlq():
    ...
    def ensure_target_tables():
        """
        Ensure all target DB tables exist and have the correct columns.
        ...
        with psycopg2.connect(source_conn_str) as source_conn, \
            ...
            for table_name, config in TABLE_CONFIGS.items():
                ...
                # 1. Fetch source column types (Needed for both CREATE and ALTER)
                ...
                with target_conn.cursor() as tgt_cur:
                    ...
                    table_exists = tgt_cur.fetchone()[0]

                    # Helper to resolve target column names and types
                    def get_target_def(col_name):
                        # Handle 'e_' prefix removal
                        tgt_name = col_name[2:] if (col_name in encrypted_cols and col_name.startswith("e_")) else col_name
                        # Get Type
                        src_type = schema_info.get(col_name, "TEXT")
                        pg_type = PG_TYPE_MAP.get(src_type.lower(), "TEXT")
                        return tgt_name, pg_type

                    if not table_exists:
                        # --- CREATE TABLE LOGIC ---
                        column_defs = []
                        for col in columns:
                            tgt_name, pg_type = get_target_def(col)
                            if col == id_column:
                                column_defs.append(f'"{tgt_name}" {pg_type} PRIMARY KEY')
                            else:
                                column_defs.append(f'"{tgt_name}" {pg_type}')
                        ...
                        # --- SCHEMA EVOLUTION (ALTER TABLE) LOGIC ---
                        ...
                        existing_target_cols = {row[0] for row in tgt_cur.fetchall()}
                        ...
                        for col in columns:
                            ...
                            if tgt_name not in existing_target_cols:
                                logger.warning(f"Schema Evolution: Adding missing column '{tgt_name}' to '{table_name}'")
                                alter_statements.append(f'ALTER TABLE "{table_name}" ADD COLUMN "{tgt_name}" {pg_type}')

                        # Execute ALTER statements
                        if alter_statements:
                            for stmt in alter_statements:
                                tgt_cur.execute(stmt)
                            logger.info(f"Updated schema for {table_name}: Added {len(alter_statements)} columns")
                        else:
                            logger.info(f"Schema for {table_name} is up to date.")

            target_conn.commit()

        logger.info("Target tables validation and schema evolution completed")
        return True
        ...
        if xcom_results:
            for result in xcom_results:
                ...
                for res in result:
                    ...
                    try:
                        with psycopg2.connect(source_conn_str) as src_conn, psycopg2.connect(target_conn_str) as tgt_conn:
                        ...
                        target_count = "ERR"
                        diff = "ERR"
                        logger.error(f"Count check failed for {table}: {e}")
                    ...
                    if status == "SUCCESS":
                        ...
                        logger.info(
                            f"✓ {table}: {rows:,} rows in {duration:.1f}s | "
